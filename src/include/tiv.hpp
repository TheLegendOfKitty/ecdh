...
std::string get_color(int flags, int r, int g, int b){
  r = clamp_byte(r);
  g = clamp_byte(g);
  b = clamp_byte(b);
  ...
  ss << (bg ? "\x1B[48;5;" : "\u001B[38;5;") << color_index << "m";
  color = ss.str();
  return color;
}
...
std::string get_image(const cimg_library::CImg<unsigned char> & image, int flags){
  std::stringstream ss;
  CharData lastCharData;
  for (int y = 0; y <= image.height() - 8; y += 8) {
    for (int x = 0; x <= image.width() - 4; x += 4) {
      CharData charData = flags & FLAG_NOOPT
        ...
        : findCharData(image, x, y, flags);
      if (x == 0 || charData.bgColor != lastCharData.bgColor)
        ss << get_color(flags | FLAG_BG, charData.bgColor[0], charData.bgColor[1], charData.bgColor[2]);
      if (x == 0 || charData.fgColor != lastCharData.fgColor)
        ss << get_color(flags | FLAG_FG, charData.fgColor[0], charData.fgColor[1], charData.fgColor[2]);
      emitCodepoint(charData.codePoint);
      lastCharData = charData;
    }
    ss << "\x1b[0m" << std::endl;
...
//std::string print(int argc, char* argv[]);

/*int main(int argc, char** argv){
  std::cout << print(argc, argv);
}*/

std::string print(/*int argc, char* argv[]*/ std::string filename, int width) {
  ...
  if (std::filesystem::is_directory(filename)) {
        ...
        if (fin) {
          ...
          std::cerr << "Can't open file (permission?): " << filename << std::endl;
        }
      }
  //file_names.push_back(filename);
  maxWidth = 4*width;
